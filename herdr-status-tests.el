...
(ert-deftest herdr-status-workspace-buffers-keep-a-dashboard-each ()
  (let ((herdr-status-buffer-name "*herdr-status*")
        (herdr-current-workspace-label-function (lambda () "arvato")))
    (should (equal (herdr-status-default-buffer-name) "*herdr-status*"))
    (should (equal (herdr-status-workspace-buffer-name) "*herdr-status: arvato*"))
    (let ((herdr-current-workspace-label-function #'ignore))
      (should (equal (herdr-status-workspace-buffer-name) "*herdr-status*")))))

(ert-deftest herdr-status-close-pane-closes-what-point-stands-for ()
  (herdr-status-tests--with-dashboard
    (let (closes detached)
      (cl-letf (((symbol-function 'herdr-api-pane-close)
                 (lambda (pane) (push pane closes)))
                ((symbol-function 'herdr-agent-resolve-session)
                 (lambda (&rest _) (user-error "none")))
                ((symbol-function 'herdr-agent-detach)
                 (lambda (session) (push session detached)))
                ((symbol-function 'yes-or-no-p) (lambda (&rest _) t))
                ((symbol-function 'herdr-status-refresh) #'ignore))
        (goto-char (point-min))
        (should (re-search-forward "^Panes " nil t))
        (forward-line 1)
        (herdr-status-close-pane)
        (should (equal closes '("%9")))
        (should-not detached)
        (goto-char (point-min))
        (should (re-search-forward "api-review" nil t))
        (herdr-status-close-pane)
        (should (equal (car closes) "%1"))))))

(ert-deftest herdr-status-project-scope-takes-the-worktrees-along ()
  (let ((herdr-project-root-function #'identity))
    (cl-letf (((symbol-function 'herdr-repository-root)
               (lambda (directory)
                 (when (string-prefix-p "/repo/" (file-name-as-directory directory))
                   "/repo"))))
      (should (equal (herdr-status--project-root "/repo") "/repo"))
      (should (equal (herdr-status--project-root "/repo/.worktrees/side") "/repo"))
      (should (equal (herdr-status--project-root "/elsewhere") "/elsewhere"))
      (let* ((default-directory "/repo/")
             (predicate (herdr-status--project-predicate)))
        (should (funcall predicate '((cwd . "/repo"))))
        (should (funcall predicate '((cwd . "/repo/.worktrees/side"))))
        (should (funcall predicate
                         '((cwd . "/repo") (foreground_cwd . "/repo/.worktrees/x"))))
        (should-not (funcall predicate '((cwd . "/elsewhere"))))
        (should-not (funcall predicate '((pane_id . "%1"))))))))

(ert-deftest herdr-status-rename-asks-for-a-pane-by-its-own-word ()
  (herdr-status-tests--with-dashboard
    (let (prompts renamed labelled)
      (cl-letf (((symbol-function 'read-string)
                 (lambda (prompt &rest _) (push prompt prompts) "chosen"))
                ((symbol-function 'herdr-agent-rename)
                 (lambda (target name) (push (list target name) renamed)))
                ((symbol-function 'herdr-api-pane-rename)
                 (lambda (pane &rest arguments)
                   (push (list pane (plist-get arguments :label)) labelled)))
                ((symbol-function 'herdr-status-refresh) #'ignore))
        (goto-char (point-min))
        (should (re-search-forward "api-review" nil t))
        (call-interactively #'herdr-status-rename)
        (should (equal prompts '("Agent name: ")))
        (should (equal (cadar renamed) "chosen"))
        (should-not labelled)
        (goto-char (point-min))
        (should (re-search-forward "%9" nil t))
        (call-interactively #'herdr-status-rename)
        (should (equal (car prompts) "Pane name: "))
        (should (equal (car labelled) '("%9" "chosen")))
...
(ert-deftest herdr-status-rows-name-the-server-they-run-on ()
  (herdr-status-tests--with-dashboard
    ...
    (goto-char (point-min))
    (should (re-search-forward "^ +● beta-work +✳ claude +beta +▣ %3 +▤ other +/ /tmp/other/$" nil t))))
...
(ert-deftest herdr-read-agent-draws-rows-without-the-dashboard-noise ()
  (herdr-status-tests--with-dashboard
    (let* ((entries (herdr-status-switch-agents))
           (affix (herdr-status--read-agent-affixation entries))
           (entry (cl-find "api-review" entries
                           :key #'herdr--entry-label :test #'equal))
           (parts (funcall affix "api-review" entry))
           (shown (nth 0 parts))
           (prefix (nth 1 parts))
           (suffix (nth 2 parts)))
      (should (equal shown "api-review"))
      (let ((glyph (herdr-status-glyph herdr-status-state-glyph)))
        (should (equal (string-trim prefix) glyph))
        (should (eq (get-text-property (string-match (regexp-quote glyph) prefix)
                                       'face prefix)
                    'herdr-status-state-working)))
      (should (string-match-p "✳ claude" suffix))
      (should (string-prefix-p "  " suffix))
      (should (string-match-p "proj" suffix))
      (should-not (string-match-p "▣" suffix))
      (should-not (string-match-p "▤" suffix))
      (should-not (string-match-p "/tmp/proj/" suffix))
      (should (text-property-not-all 0 (length suffix) 'face nil suffix)))))

(ert-deftest herdr-read-agent-cuts-a-long-name-for-the-row-alone ()
  (herdr-status-tests--with-dashboard
    (let* ((long (make-string 200 ?x))
           (entries (list `((name . ,long) (agent . "claude")
                            (agent_status . "idle") (terminal_id . "t9")
                            (server_key . "/tmp/alpha.sock") (cwd . "/tmp/proj/"))))
           (herdr-read-agent-name-width 20)
           (parts (funcall (herdr-status--read-agent-affixation entries)
                           long (car entries))))
      (should (= (string-width (nth 0 parts)) 20))
      (should (string-suffix-p "…" (nth 0 parts)))
      (should (string-prefix-p "  " (nth 2 parts))))))
    ...
    (let ((entries (herdr-status-switch-agents))
      ...
      (cl-letf (((symbol-function 'completing-read)
        ...
        (herdr-read-entry "Switch: " entries
                          :affixation (lambda (candidate _entry)
                                        (list (substring candidate 0 3) "<" ">"))
                          :group nil))
...
(ert-deftest herdr-project-status-rejects-a-missing-project-before-changing-view ()
  (let ((herdr-project-root-function (lambda (&optional _) nil)))
    (cl-letf (((symbol-function 'herdr-repository-root) #'ignore))
      (herdr-status-tests--with-dashboard
        (let ((herdr-status-buffer-name (buffer-name))
              (text (buffer-string)))
...
(ert-deftest herdr-status-takes-the-nerd-font-glyphs-down-to-the-text ()
  (let ((herdr-status-nerd-font t))
    (let ((column (herdr-status--harness-column '((agent . "claude")) 6)))
      (should (memq 'herdr-status-nerd-glyph
                    (ensure-list (get-text-property 0 'font-lock-face column)))))
    (let ((cell (herdr-status--field-column 'directory "/tmp/x/" 7)))
      (should (memq 'herdr-status-nerd-glyph
                    (ensure-list (get-text-property 0 'font-lock-face cell))))
      (should-not (memq 'herdr-status-nerd-glyph
                        (ensure-list
                         (get-text-property (1- (length cell))
                                            'font-lock-face cell))))))
  (let ((herdr-status-nerd-font nil))
    (should-not (memq 'herdr-status-nerd-glyph
                      (ensure-list
                       (get-text-property
                        0 'font-lock-face
                        (herdr-status--harness-column '((agent . "claude")) 6)))))))

(ert-deftest herdr-status-marks-the-message-field-with-the-harness-it-writes-to ()
  (let ((herdr-status-nerd-font t)
        (target '("/servers/a.sock" . "shared")))
    (cl-letf (((symbol-function 'herdr-agent-find)
               (lambda (_server _terminal) nil))
              ((symbol-function 'herdr-sessions)
               (lambda ()
                 `(((server_key . "/servers/a.sock") (terminal_id . "shared")
                    (agent . "codex"))))))
      (let ((prefix (herdr-message--field-prefix target)))
        (should (string-prefix-p "" prefix))
        (should (equal (get-text-property 0 'face prefix)
                       '(herdr-status-nerd-glyph herdr-status-harness-codex)))))
    (cl-letf (((symbol-function 'herdr-agent-find)
               (lambda (_server _terminal)
                 (herdr-agent--make-session :kind "claude"))))
      (let ((prefix (herdr-message--field-prefix target)))
        (should (string-prefix-p "" prefix))
        (should (equal (get-text-property 0 'face prefix)
                       '(herdr-status-nerd-glyph herdr-status-harness-claude)))))
    (cl-letf (((symbol-function 'herdr-agent-find)
               (lambda (_server _terminal)
                 (herdr-agent--make-session :kind "pi"))))
      (should-not (herdr-message--field-prefix target)))))

(ert-deftest herdr-status-field-glyphs-fall-back-to-what-the-display-shows ()
  (let ((herdr-status-nerd-font t)
        (herdr-status-field-glyphs '((branch "" "⎇" "@") (directory "⌂" ""))))
...
(ert-deftest herdr-status-draws-the-model-an-agent-reports ()
  (let* ((reported '((agent . "claude") (pane_id . "%1")
         ...
         (workspaces (make-hash-table :test #'equal)))
    (should (equal (herdr-status--model reported) "Opus 5"))
    (should-not (herdr-status--model blank))
    (should-not (herdr-status--model silent))
    (should (equal (herdr-status--model-key silent) ""))
  ...
  (herdr-status-tests--with-dashboard
    (let ((herdr-status-new-harness "claude")
      ...
      (cl-letf (((symbol-function 'herdr-agent-start)
        ...
        (should (re-search-forward "⌬ codex" nil t))
        (should (equal (alist-get 'agent (herdr-status-entry-at-point)) "codex"))
        (should (equal (herdr-status--new-harness) "codex"))
        (herdr-status-new-agent)
        (should (equal (car starts) '("codex" nil "/tmp/proj/" "alpha")))
        ...
        (cl-letf (((symbol-function 'completing-read)
          ...
          (call-interactively #'herdr-status-new-agent-of-harness))
        (should (equal (car starts) '("claude" nil "/tmp/proj/" "alpha")))))))
