...
(ert-deftest herdr-claude-attachment-uses-the-registered-harness-adapter ()
  (let* ((terminal-id "term-claude")
         ...
         (herdr-agent-harnesses (copy-tree herdr-agent-harnesses))
         phases session)
...
(ert-deftest herdr-claude-connect-p-follows-agent-status ()
  (let ((idle '((agent_status . "idle")))
        (working '((agent_status . "working")))
        (herdr-attach-takeover t))
    (let ((herdr-claude-connect-on-adopt 'idle))
      (should (herdr-claude--connect-p idle))
      (should-not (herdr-claude--connect-p working)))
    (let ((herdr-claude-connect-on-adopt t))
      (should (herdr-claude--connect-p working)))
    (let ((herdr-claude-connect-on-adopt nil))
      (should-not (herdr-claude--connect-p idle)))
    (let ((herdr-claude-connect-on-adopt t)
